...
```bash
git clone https://github.com/<your-username>/sst-ids.git
cd sst-ids
...
The experiments use three public datasets, which should be downloaded from their
official sources and placed in a folder pointed to by `DATA_DIR` in `config.py`
(default `./Datsets`):
...
- **TON_IoT** — `ton-iot.csv` (network telemetry).
...
for CIC-IDS-2017) for fast, exact reproduction. The default configuration in
...
`run_all.py` writes CSV/LaTeX tables and SHAP figures to `./outputs`: hyperparameter
settings, within-dataset and baseline comparisons (mean ± standard deviation over
seeds), paired-t-test significance, cross-dataset transfer, global and local SHAP
attributions over the selected behavioral features, per-dataset feature lists, and
per-flow inference latency / model size.
...
- Metrics are computed only where they are well defined. For an evaluation set
  ...
  pooled multi-class setup (`load_cic_pooled`) is recommended for within-dataset
  evaluation.
...
```bibtex
@article{sstids2026,
  title   = {SST-IDS: A Self-Supervised Transformer Framework for Explainable
             Intrusion Detection Across Heterogeneous Network Datasets},
