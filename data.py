...
data.py
=======
Loading, cleaning, preprocessing, and RF-SHAP feature selection for the IDS
datasets, matched to the exact column layout of the public distributions:
...
# ---------------------------------------------------------------------------
def check_datasets(datasets=("cic", "unsw")):
    """Verify the expected CSV files exist under config.DATA_DIR.

    Raises FileNotFoundError with the full list of expected paths so a user
    (or reviewer) knows exactly where to place the downloaded files.
    """
    expected = []
    if "cic" in datasets:
        expected += [C.DATA_DIR / f for f in C.CIC_FILES.values()]
    if "unsw" in datasets:
        expected += [C.DATA_DIR / C.UNSW_TRAIN, C.DATA_DIR / C.UNSW_TEST]
    missing = [p for p in expected if not p.exists()]
    if missing:
        lines = "\n".join(f"  - {p}" for p in expected)
        raise FileNotFoundError(
            f"Dataset files not found under DATA_DIR={C.DATA_DIR.resolve()}.\n"
            f"Download CIC-IDS-2017 (MachineLearningCVE flow CSVs) and "
            f"UNSW-NB15 (training/testing set CSVs) from their official "
            f"providers and place them so these paths exist:\n{lines}\n"
            f"(DATA_DIR can be changed in config.py.)")
...
def _read_csv_clean(path, label_col_candidates):
    ...
    if not Path(path).exists():
        raise FileNotFoundError(
            f"Dataset file not found: {path}. Place the CSV under "
            f"DATA_DIR={C.DATA_DIR.resolve()} (see config.py and README.md).")
...
def select_features_rf_shap(X_df, y, top_k, random_state=0, max_shap_rows=1000,
                            strict=False):
    """Rank features by mean |SHAP| from a Random Forest. Returns
    (selected_names, ranking_df); ranking_df records the method actually used.

    strict=True (the reproduction path): raise RuntimeError if SHAP is
    unavailable or fails, so the reported method is exactly RF-SHAP as stated
    in the paper. strict=False (exploratory runs): fall back to Random Forest
    impurity importance with a loud warning.
    ...
    if not _HAVE_SHAP:
        if strict:
            raise RuntimeError(
                "SHAP is required for the paper's RF-SHAP feature selection "
                "but is not installed (pip install shap). Refusing to fall "
                "back to impurity importance in strict/reproduction mode.")
        warnings.warn("shap not installed; using RF impurity importance. "
                      "This does NOT reproduce the paper's RF-SHAP selection.")
    elif X_df.shape[0] > 0:
        try:
            ...
            if strict:
                raise RuntimeError(
                    f"RF-SHAP feature ranking failed ({e}). Refusing to fall "
                    f"back to impurity importance in strict/reproduction mode."
                ) from e
            warnings.warn(f"SHAP ranking failed ({e}); using impurity importance. "
                          f"This does NOT reproduce the paper's RF-SHAP selection.")
...
def prepare_split(X, y, cats, top_k, seed=0, strict_shap=False):
    ...
    strict_shap=True forbids the impurity-importance fallback (reproduction path)."""
...
def prepare_within_dataset(dataset, seed=0, sample_per_file=None, strict_shap=False):
    ...
    return prepare_split(X, y, cats, top_k, seed, strict_shap=strict_shap)
