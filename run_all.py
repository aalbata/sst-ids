...
run_all.py
==========
Orchestrates the full SST-IDS experiment suite on the datasets you shared and
writes paper-ready artifacts into OUTPUT_DIR:

  table5_hyperparameters.{csv,tex}      <- fills Table 5
  table6_within_dataset.{csv,tex}       <- within-dataset metrics, mean+/-std
  table7_transfer.{csv,tex}             <- CIC->UNSW, with & without identity feats
  table8_baseline_comparison.{csv,tex}  <- all models, mean+/-std
  table9_significance.{csv,tex}         <- paired t-tests, sig + non-sig
  table10_shap_global.csv + *.png       <- real SHAP on selected behavioral feats
...
All numbers are computed from YOUR data. Nothing is hard-coded into the paper.
"""
...
def stage_transfer(out, cfg, sample_per_file, seeds):
    rows = []
    for drop in (False, True):                       # before / after identity removal
        C.RUN.drop_identity_features = drop
        accs = []
        for s in seeds:
            met, fmap = P.transfer_cic_to_unsw(cfg, seed=s,
                                               sample_per_file=sample_per_file or 20000)
            accs.append(met)
        mean = {k: np.nanmean([m[k] for m in accs]) for k in ("accuracy", "f1", "auc")}
        std = {k: np.nanstd([m[k] for m in accs]) for k in ("accuracy", "f1", "auc")}
        rows.append({"Setting": "without identity features" if drop else "all features",
                     "Accuracy": _fmt(mean["accuracy"], std["accuracy"]),
                     "F1": _fmt(mean["f1"], std["f1"]), "AUC": _fmt(mean["auc"], std["auc"])})
    C.RUN.drop_identity_features = False
    write_table(pd.DataFrame(rows), str(out / "table7_transfer"),
                "Cross-dataset transfer (train CIC-IDS-2017, test UNSW-NB15) before "
                "and after removing identity/port features.", "tab:cross_dataset_results")
    (out / "transfer_feature_map.json").write_text(json.dumps(P.CIC_TO_UNSW, indent=2))
    print("[table7] transfer (with/without identity features) written")
...
def stage_explain_and_cost(out, cfg, sample_per_file, seeds, do_shap):
    ...
    # deployment cost: SST-IDS vs RF vs XGBoost
    rows = []
    b = P.benchmark_latency_memory(model, prep["X_test"], dev, is_torch=True)
    rows.append({"Model": "SST-IDS", "Per-flow (ms)": f"{b['per_flow_ms']:.3f}",
    ...
    for name in ("Random Forest", "XGBoost"):
        try:
            ...
            rows.append({"Model": name, "Per-flow (ms)": f"{bb['per_flow_ms']:.3f}",
                         "Params (MB)": "--"})
